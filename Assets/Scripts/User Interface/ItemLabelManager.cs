...
namespace User_Interface
{
    public class ItemLabelManager : MonoBehaviour
    {
        ...
        public void RegisterLabel(ItemLabel label)
        {
            _labels.Add(label);
        }

        public void UnRegisterLabel(ItemLabel label)
        {
        ...
        private void LateUpdate()
        {
            ...
            for (var i = 0; i < _labels.Count - 1; i++)
            {
                ...
                if (screenLabelRectA.Overlaps(screenLabelRectB))
                {
                    ...
                    var screenPosB = _mainCamera.WorldToScreenPoint(_labels[i + 1].transform.position);
                    ...
                    var targetScreenY = screenLabelRectA.yMax + screenLabelRectB.height * 0.5f;

                    // 3. Update screen position.
                    screenPosB.y = targetScreenY;

                    // 4. Convert back to world space using the original depth (z) of label B.
                    var newWorldPos = _mainCamera.ScreenToWorldPoint(screenPosB);
