            ...
            switch (property.propertyType)
            {
                ...
                case SerializedPropertyType.Generic:
                    ...
                    if (property.serializedObject.targetObject is not Component)
                        return new HelpBox("The DrawHandle Attribute can only be used with GameObjects",
                            HelpBoxMessageType.Error);

                    if (drawHandleAttribute.HandleSpace == Space.Self &&
                        property.propertyType is SerializedPropertyType.Vector2Int or SerializedPropertyType.Vector3Int)
                        return new HelpBox("<b>Vector2Int</b> and <b>Vector3Int</b> handles don't support local space",
                    ...
                    if (!EditorHandles.handleProperties.ContainsKey(property.propertyPath))
                    {
                        ...
                        if (property.propertyType is SerializedPropertyType.Rect or SerializedPropertyType.RectInt)
                            EditorHandles.boundsHandleList.Add(property.propertyPath,
                                new BoxBoundsHandle
                                    { axes = PrimitiveBoundsHandle.Axes.X | PrimitiveBoundsHandle.Axes.Y });
                        else if (property.propertyType is SerializedPropertyType.Bounds
