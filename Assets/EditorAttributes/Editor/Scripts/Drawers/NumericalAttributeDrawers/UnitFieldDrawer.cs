    ...
    public class UnitFieldDrawer : PropertyDrawerBase
    {
        public override VisualElement CreatePropertyGUI(SerializedProperty property)
        {
            if (!IsSupportedType(property.propertyType))
                return new HelpBox("The UnitField Attribute can only be attached to numeric types",
            ...
            var numericField = CreateFieldForType(numericType, property.displayName,
                GetConvertedPropertyValue(property, convertedUnit.conversion), property.hasMultipleDifferentValues);

            numericField.TrackPropertyValue(property,
                (trackedProperty) => SetFieldValue(numericField,
                    GetConvertedPropertyValue(trackedProperty, convertedUnit.conversion)));
            RegisterValueChangedCallbackByType(numericType, numericField,
            ...
            numericField.RegisterCallbackOnce<GeometryChangedEvent>((callback) =>
            {
                var inputFields = numericField
                    .Query<VisualElement>(className: TextInputBaseField<Void>.inputUssClassName).ToList();
        ...
        private void SetNumericPropertyValue(SerializedProperty property, object value, double conversion)
        {
            ...
            {
            } // If this is thrown then it means that the value is a vector or rect type, which we handle separately
                ...
                default:
                    switch (property.propertyType)
                    {
                        case SerializedPropertyType.Vector2:
                            ...
                            property.vector2Value = new Vector2(vector2Value.x * (float)conversion,
                                vector2Value.y * (float)conversion);
                            break;

                        case SerializedPropertyType.Vector3:
                            var vector3Value = (Vector3)value;
                            property.vector3Value = new Vector3(vector3Value.x * (float)conversion,
                                vector3Value.y * (float)conversion, vector3Value.z * (float)conversion);
                            break;

                        case SerializedPropertyType.Vector4:
                            var vector4Value = (Vector4)value;
                            property.vector4Value = new Vector4(vector4Value.x * (float)conversion,
                                vector4Value.y * (float)conversion, vector4Value.z * (float)conversion,
                                vector4Value.w * (float)conversion);
                            break;

                        case SerializedPropertyType.Rect:
                            var rectValue = (Rect)value;
                            property.rectValue = new Rect(rectValue.x * (float)conversion,
                                rectValue.y * (float)conversion, rectValue.width * (float)conversion,
                                rectValue.height * (float)conversion);
                            break;

                        case SerializedPropertyType.Vector2Int:
                            var vector2IntValue = (Vector2Int)value;
                            property.vector2IntValue = new Vector2Int((int)(vector2IntValue.x * conversion),
                                (int)(vector2IntValue.y * conversion));
                            break;

                        case SerializedPropertyType.Vector3Int:
                            var vector3IntValue = (Vector3Int)value;
                            property.vector3IntValue = new Vector3Int((int)(vector3IntValue.x * conversion),
                                (int)(vector3IntValue.y * conversion), (int)(vector3IntValue.z * conversion));
                            break;

                        case SerializedPropertyType.RectInt:
                            var rectIntValue = (RectInt)value;
                            property.rectIntValue = new RectInt((int)(rectIntValue.x * conversion),
                                (int)(rectIntValue.y * conversion), (int)(rectIntValue.width * conversion),
