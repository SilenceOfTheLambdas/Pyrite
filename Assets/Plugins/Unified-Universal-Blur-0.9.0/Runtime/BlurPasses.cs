...
namespace Unified.UniversalBlur.Runtime
{
    public static class BlurPasses
    {
        public static void KawaseExecutePass<TPass, T>(TPass data, T cmd)
            where TPass : IPassData where T : IWrappedCommandBuffer
        {
            ...
            if (blurConfig.Iterations % 2 == 1) (source, destination) = (destination, source);
            ...
            where T : IWrappedCommandBuffer
        {
            ...
            mpb.SetVector(Constants.BlurParamsId,
                new Vector4(blurConfig.Intensity, blurConfig.Scale, blurConfig.Downsample, blurConfig.Offset));
