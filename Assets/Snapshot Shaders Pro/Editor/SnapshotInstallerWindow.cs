...
namespace SnapshotShaders
{
    public class SnapshotInstallerWindow : EditorWindow
    {
        ...
        {
            var editorWindow = GetWindow(typeof(SnapshotInstallerWindow), false, "Snapshot Pro Installer", true);

            editorWindow.autoRepaintOnSceneChange = true;
            editorWindow.ShowAuxWindow();

            editorWindow.position = new Rect(Screen.currentResolution.width / 2f - width * 0.5f,
                Screen.currentResolution.height / 2f - height * 0.7f, width, height);
        ...
        private void OnGUI()
        {
            ...
            if (bannerTexture == null) bannerTexture = Resources.Load<Texture2D>("InstallerBanner");
            ...
            if (installedPipelines.Contains(SnapshotInstaller.Pipeline.BuiltinPostProcess))
                if (compatiblePipelines.Contains(SnapshotInstaller.Pipeline.URP) ||
                    compatiblePipelines.Contains(SnapshotInstaller.Pipeline.HDRP)
                    || installedPipelines.Contains(SnapshotInstaller.Pipeline.URP) ||
                    installedPipelines.Contains(SnapshotInstaller.Pipeline.HDRP))
                {
                    GUILayout.Space(5);
                    ...
                    using (new EditorGUILayout.VerticalScope(pipelineBox))
                    {
                        EditorGUILayout.LabelField(
                            "<b><color=\"red\">Warning:</color></b> The built-in pipeline package is installed but you are using URP/HDRP. Consider deleting the \"Built-in Pipeline\" folder.",
                            infoStyle);
                    }
                }
            ...
            else if (compatiblePipelines.Contains(SnapshotInstaller.Pipeline.URP))
            {
                using (new EditorGUILayout.VerticalScope(pipelineBox))
                {
                    ...
                    EditorGUILayout.LabelField(
                        "<i>Snapshot Shaders Pro</i> for URP is not installed (it may have been moved or some files may have been deleted).",
                        infoStyle);
            ...
            else if (compatiblePipelines.Contains(SnapshotInstaller.Pipeline.HDRP))
            {
                using (new EditorGUILayout.VerticalScope(pipelineBox))
                {
                    ...
                    EditorGUILayout.LabelField(
                        "<i>Snapshot Shaders Pro</i> for HDRP is not installed (it may have been moved or some files may have been deleted).",
                        infoStyle);
            ...
            if (compatiblePipelines.Contains(SnapshotInstaller.Pipeline.BuiltinAlone))
            {
                using (new EditorGUILayout.VerticalScope(pipelineBox))
                {
                    ...
                    EditorGUILayout.LabelField(
                        "<i>Snapshot Shaders Pro</i> for the built-in render pipeline requires the Post Processing Stack v2 package to be installed.",
                        infoStyle);
            ...
            else
            {
                if (installedPipelines.Contains(SnapshotInstaller.Pipeline.BuiltinPostProcess))
                    ...
                    using (new EditorGUILayout.VerticalScope(pipelineBox))
                    {
                        ...
                        EditorGUILayout.LabelField(
                            "<i>Snapshot Shaders Pro</i> for the built-in pipeline not detected (it may have been moved or some files may have been deleted).",
                            infoStyle);
