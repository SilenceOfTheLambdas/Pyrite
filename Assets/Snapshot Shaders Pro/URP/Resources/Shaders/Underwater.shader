        ...
        Pass
        {
            ...
            float4 frag(Varyings i) : SV_TARGET
            {
                ...
                float3 worldNormal = normalize(
                    SAMPLE_TEXTURE2D_X(_CameraNormalsTexture, sampler_LinearClamp, i.texcoord));

                float noise = perlin2D((worldPos.xz + _Time.y * _CausticsNoiseSpeed) * _CausticsNoiseScale) *
                    _CausticsNoiseStrength;

                float4 triplanar1 = triplanarSample(_CausticsTexture, sampler_LinearRepeat,
                                                    (worldPos + _CausticsScrollVelocity1 * _Time.y + noise) *
                                                    _CausticsTiling.x, worldNormal, 1.0f);
                float4 triplanar2 = triplanarSample(_CausticsTexture, sampler_LinearRepeat,
                                                    (worldPos + _CausticsScrollVelocity2 * _Time.y + noise) *
                                                    _CausticsTiling.y, worldNormal, 1.0f);
