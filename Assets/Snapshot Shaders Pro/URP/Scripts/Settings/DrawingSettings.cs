namespace SnapshotShaders.URP
{
    ...
    public sealed class DrawingSettings : VolumeComponent, IPostProcessComponent
    {
        ...
        [Tooltip("Drawing overlay texture.")] public TextureParameter drawingTex = new(null);

        [Tooltip("Time taken (in seconds) per animation cycle. Set to zero for no animation.")]
        public ClampedFloatParameter animCycleTime = new(0.75f, 0.0f, 5.0f);
        ...
        public ClampedFloatParameter tiling = new(25.0f, 1.0f, 50.0f);

        [Tooltip("Amount of UV smudging based on drawing texture colour values.")]
        public ClampedFloatParameter smudge = new(0.001f, 0.0f, 5.0f);

        [Tooltip("Pixels past this depth threshold will not be 'drawn on'.")]
        public ClampedFloatParameter depthThreshold = new(0.99f, 0.0f, 1.01f);
